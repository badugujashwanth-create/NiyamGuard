...
@router.get("/plans", dependencies=[Depends(require_roles("officer", "reviewer"))])
def list_plans() -> dict:
    return {"success": True, "plans": [item.model_dump() for item in read_store().propagation_plans]}


@router.get("/tasks", dependencies=[Depends(require_roles("officer", "reviewer"))])
def list_tasks() -> dict:
    return {"success": True, "tasks": [item.model_dump() for item in propagation_service.list_tasks()]}


@router.get("/tasks/{task_id}", dependencies=[Depends(require_roles("officer", "reviewer"))])
...
@router.post("/tasks/{task_id}/apply-demo-patch", dependencies=[Depends(require_roles("officer", "reviewer"))])
...
@router.post("/tasks/{task_id}/mark-completed", dependencies=[Depends(require_roles("officer", "reviewer"))])
...
@router.post("/tasks/{task_id}/mark-manual", dependencies=[Depends(require_roles("officer", "reviewer"))])
