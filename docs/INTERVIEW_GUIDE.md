...
The current `main` commit passes 273 backend and 60 frontend tests, plus a frontend production build and dependency/security checks in CI. A 5:37 browser-driven walkthrough exercises the real synthetic workflow, evidence-derived readiness, policy lineage, knowledge relationships, and audit evidence. Explain that this is strong local evidence, not proof of production integrations; the Docker daemon and hosted service still require external verification.
