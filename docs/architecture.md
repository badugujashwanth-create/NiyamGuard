...
Local development defaults to SQLite and compatibility JSON-backed demo store helpers. PostgreSQL is supported through `DATABASE_URL` for production-style deployments. The fourteen flagship policy-review collections use typed relational tables; the wider seeded/demo JSON remains a compatibility mirror and is not a second production authority.
