...
| Fresh SQLite Alembic migration and normalized seed/load round trip | Pass: migrations through `20260807_0009`, readiness, review/publication audit row counts, and candidate evidence columns verified locally |
...
3. Replace the remaining non-core generic JSON payload rows with normalized relational records and domain transaction boundaries for a true pilot; the ten flagship policy-review collections are now typed and foreign-key constrained, while the serialized store remains a compatibility mirror.
