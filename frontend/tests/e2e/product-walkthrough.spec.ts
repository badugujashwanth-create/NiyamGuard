...
test("record the complete product simulation", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Department Readiness" })).toBeVisible();
  await scene(page, 8_000);
  await page.getByRole("heading", { name: "Department Readiness" }).scrollIntoViewIfNeeded();
  await expect(page.getByText("Revenue").last()).toBeVisible();
  await scene(page, 10_000);

  await page.goto(`${baseUrl}/admin/policy-updates`, { waitUntil: "networkidle" });
  await expect(page.getByRole("heading", { name: "Policy Lineage" })).toBeVisible();
  await page.getByRole("heading", { name: "Policy Lineage" }).scrollIntoViewIfNeeded();
  await scene(page, 8_000);
  await page.getByText("rule_001 / v3").scrollIntoViewIfNeeded();
  await expect(page.getByText("version_rule_001_2")).toBeVisible();
  await scene(page, 10_000);

  await page.goto(`${baseUrl}/admin/knowledge-base`, { waitUntil: "networkidle" });
  await expect(page.getByRole("heading", { name: "Knowledge Relationships" })).toBeVisible();
  await page.getByLabel(/Search sources, rules, departments/).fill("Public FAQ");
  await expect(page.getByText(/Public FAQ/).last()).toBeVisible();
  await scene(page, 15_000);
